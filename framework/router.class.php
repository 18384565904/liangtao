...
class router
{
    ...
    protected function setAppRoot($appName = 'demo', $appRoot = '')
    {
        if(empty($appRoot))
        {
            $this->appRoot = $this->basePath . $this->pathFix . $appName . $this->pathFix;;
        }
        else
        {
    ...
    public function setClientLang($lang = '')
    {
        ...
        {
            $this->clientLang = $this->clientLang;
    ...
    public function setClientTheme($theme = '')
    {
        ...
        {
            $this->clientTheme = $this->clientTheme;
    ...
    public function getModulePath($moduleName = '')
    {
        ...
        return $this->getModuleRoot() . trim($moduleName) . $this->pathFix;
    }
    ...
    {
        return $this->getModuleRoot() . trim($moduleName) . $this->pathFix . 'ext' . $this->pathFix . $ext . $this->pathFix;
    }
    ...
    {
        global $config, $dbh,$dbh2, $slaveDBH;

        if(isset($config->db->host))      $this->dbh      = $dbh      = $this->connectByPDO($config->db);
        //if(isset($config->db->host))      $this->dbh2      = $dbh2      = $this->connectByPDO2($config->db);
    ...
    private function connectByPDO2($params)
    {
    	if(!isset($params->driver)) self::error('no pdo driver defined, it should be mysql or sqlite', __FILE__, __LINE__, $exit = true);
        if(!isset($params->user)) return false;
        if($params->driver == 'mysql')
        ...
        try 
        {
            $dbh = new PDO($dsn, $params->user2, $params->password2, array(PDO::ATTR_PERSISTENT => $params->persistant));
            $dbh->setAttribute(PDO::ATTR_DEFAULT_FETCH_MODE, PDO::FETCH_OBJ);
            $dbh->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $dbh->exec("SET NAMES {$params->encoding}");
            if(isset($params->strictMode) and $params->strictMode == false) $dbh->exec("SET @@sql_mode= ''");
